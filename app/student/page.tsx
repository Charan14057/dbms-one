...
export default function StudentHub() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!error) { setShowVault(false); fetchScholarSystem(); }
    setSubmitting(false);
  };
    ...
    <div className="flex flex-col lg:flex-row min-h-screen bg-[#09090B] text-[#FAFAFA] font-sans selection:bg-emerald-500/30 overflow-x-hidden">
            ...
            <div className="flex-1 space-y-12">
              <div className="space-y-6">
                ...
                <div className="space-y-4">
                  ...
                  <button onClick={() => { setShowPassModal(true); setShowMobileSidebar(false); }} className="w-full bg-white/5 border border-white/5 p-5 rounded-[28px] flex items-center gap-4 hover:bg-white/10 transition-all text-left">
              ...
              <div className="space-y-6">
                <label className="text-[10px] font-black uppercase tracking-[0.5em] text-zinc-600">Metrics</label>
                <div className="grid grid-cols-1 gap-4">
                  <div className="bg-[#121214] border border-white/5 p-8 rounded-[40px] text-center"><h4 className="text-6xl font-black text-emerald-500 tracking-tighter">{stats.pubs}</h4><p className="text-[10px] font-black uppercase text-zinc-700 mt-2">Publications</p></div>
                  <div className="bg-[#121214] border border-white/5 p-8 rounded-[40px] text-center"><h4 className="text-6xl font-black text-blue-500 tracking-tighter">{stats.projs}</h4><p className="text-[10px] font-black uppercase text-zinc-700 mt-2">Active Projects</p></div>
                </div>
              </div>
            </div>
            <button onClick={() => { localStorage.clear(); router.replace('/'); }} className="flex items-center gap-4 p-5 text-zinc-500 hover:text-red-400 font-bold mt-auto transition-all"><LogOut size={22} /> <span className="text-lg uppercase italic font-black">Exit Hub</span></button>
          </motion.nav>
        )}
      </AnimatePresence>
      ...
      <main className="flex-1 overflow-y-auto p-6 lg:p-16 relative custom-scrollbar">
        ...
        <div className="space-y-10 pb-20">
          ...
          {history.length === 0 ? (
            <div className="py-24 text-center border-2 border-dashed border-white/5 rounded-[40px] lg:rounded-[60px] bg-white/[0.01]"><Activity className="mx-auto text-zinc-800 mb-6" size={64} /><p className="text-zinc-600 font-black uppercase text-xl lg:text-2xl italic tracking-[0.2em]">Zero Historical Data.</p></div>
          ) : (
                 ...
                 <div key={idx} className="bg-white/[0.01] border border-white/5 rounded-[32px] lg:rounded-[48px] overflow-hidden group hover:border-emerald-500/10 transition-all">
                    <div onClick={() => setExpandedId(expandedId === item.id ? null : item.id)} className="p-6 lg:p-10 flex items-center justify-between cursor-pointer hover:bg-white/[0.02]">
                      <div className="flex items-center gap-6 lg:gap-10">
                         <div className="text-center w-12 lg:w-16"><p className="text-[8px] lg:text-[10px] font-black uppercase text-zinc-600 mb-1">{new Date(item.created_at).toLocaleString('default', { month: 'short' })}</p><h4 className="text-2xl lg:text-4xl font-black text-white leading-none tracking-tighter">{new Date(item.created_at).getDate()}</h4></div>
                        ...
                        <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="bg-black/20 border-t border-white/5">
                          <div className="p-6 lg:p-12 grid grid-cols-1 md:grid-cols-2 gap-8 lg:gap-12">
                            ...
                            <div className="space-y-8">
                               ...
                               {item.associated_project && <div className="bg-emerald-500/5 p-6 rounded-3xl border border-emerald-500/10 text-center"><p className="text-[8px] lg:text-[9px] font-black uppercase text-emerald-600 mb-1">Linked Identity</p><p className="text-base lg:text-lg font-bold text-white italic">{item.associated_project}</p></div>}
                            </div>
                          </div>
                        </motion.div>
                      )}
