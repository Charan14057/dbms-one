...
import { 
  ...
  Link, GraduationCap, Mail, Code2, ExternalLink, Lock, CheckCircle2, Loader2, X, Menu 
} from 'lucide-react';
...
export default function MentorHub() {
  ...
  const handlePasswordUpdate = async (e: React.FormEvent) => {
    ...
    if (!error) { alert("SUCCESS: Mentor Access Key Updated."); setShowPassModal(false); }
    ...
    <div className="flex flex-col lg:flex-row min-h-screen bg-[#09090B] text-[#FAFAFA] font-sans selection:bg-blue-500/30 overflow-x-hidden">
      ...
      <AnimatePresence>
        {(showMobileSidebar || (typeof window !== 'undefined' && window.innerWidth >= 1024)) && (
          <motion.nav 
            ...
            className={`fixed lg:relative z-[70] w-80 h-full border-r border-white/5 bg-[#0C0C0E] p-10 flex flex-col flex-shrink-0 transition-transform lg:translate-x-0 ${showMobileSidebar ? 'translate-x-0' : '-translate-x-full lg:flex'}`}
          >
            <button onClick={() => setShowMobileSidebar(false)} className="lg:hidden absolute top-8 right-8 text-zinc-500"><X size={28}/></button>
            <div className="flex items-center gap-4 mb-16 px-2 hidden lg:flex"><div className="w-12 h-12 bg-blue-600 rounded-2xl flex items-center justify-center shadow-xl"><ShieldCheck size={26} className="text-white" /></div><span className="text-2xl font-black tracking-tighter uppercase leading-none italic">DBMS<br/><span className="text-blue-500">ONE</span></span></div>
              ...
              <div className="space-y-6">
                ...
                <div className="space-y-4">
                  ...
                  <button onClick={() => { setShowPassModal(true); setShowMobileSidebar(false); }} className="w-full bg-white/5 border border-white/5 p-5 rounded-[28px] flex items-center gap-4 hover:bg-white/10 transition-all text-left group">
                    ...
                    <div><p className="text-[8px] font-black text-zinc-600 uppercase leading-none">Security</p><p className="text-xs font-bold text-white italic">Update Access Key</p></div>
                  </button>
                </div>
              </div>
              <div className="space-y-6">
                <label className="text-[10px] font-black uppercase tracking-[0.5em] text-zinc-600">Supervision Load</label>
                <div className="bg-blue-600/10 border border-blue-500/20 p-10 rounded-[40px] text-center"><h4 className="text-6xl lg:text-7xl font-black text-blue-500 tracking-tighter">{mentorStats.totalSubmissions}</h4><p className="text-[10px] font-black uppercase text-blue-500/60 mt-3 leading-none">Records Supervised</p></div>
              </div>
            </div>
            <button onClick={() => { localStorage.clear(); router.replace('/'); }} className="flex items-center gap-4 p-5 text-zinc-500 hover:text-red-400 font-bold mt-auto transition-all group"><LogOut size={22} className="group-hover:-translate-x-1 transition-transform" /> <span className="text-lg uppercase italic font-black">Disconnect</span></button>
          </motion.nav>
        )}
      </AnimatePresence>
      ...
      <main className="flex-1 overflow-y-auto p-6 lg:p-16 relative custom-scrollbar">
        <header className="flex flex-col xl:flex-row justify-between items-start xl:items-center mb-12 gap-10">
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}><h1 className="text-6xl lg:text-8xl font-black tracking-tighter italic text-white leading-none">Command.</h1><p className="text-zinc-500 text-lg lg:text-xl font-medium mt-3 uppercase tracking-[0.3em] italic">Research Supervision Feed</p></motion.div>
          <div className="relative w-full md:w-[400px]"><Search className="absolute left-6 top-1/2 -translate-y-1/2 text-zinc-600" size={24} /><input className="w-full bg-white/5 border border-white/10 rounded-[28px] py-4 lg:py-5 pl-16 pr-8 text-xl text-white outline-none focus:ring-2 focus:ring-blue-500/20 placeholder:text-zinc-800 italic shadow-inner" placeholder="Search Title / Scholar..." onChange={(e) => setSearchTerm(e.target.value)} /></div>
        </header>

        <div className="space-y-6 pb-20">
          {supervisionList.filter(item => (item.title + item.student_name).toLowerCase().includes(searchTerm.toLowerCase())).length === 0 ? (
            <div className="py-24 text-center border-2 border-dashed border-white/5 rounded-[40px] lg:rounded-[60px] bg-white/[0.01]"><Activity className="mx-auto text-zinc-800 mb-6" size={64} /><p className="text-zinc-600 font-black uppercase text-xl lg:text-2xl italic tracking-[0.2em]">No Supervised Data Found.</p></div>
          ) : (
            supervisionList.filter(item => (item.title + item.student_name).toLowerCase().includes(searchTerm.toLowerCase())).map((item, idx) => (
              <motion.div key={idx} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="bg-[#121214] border border-white/5 rounded-[32px] lg:rounded-[40px] overflow-hidden group transition-all">
                <div onClick={() => toggleExpand(item.id)} className="p-6 lg:p-8 cursor-pointer flex items-center justify-between hover:bg-white/[0.02] transition-all">
                  <div className="flex items-center gap-6 lg:gap-10">
                    <div className="text-center w-12 lg:w-16"><p className="text-[8px] lg:text-[9px] font-black uppercase text-zinc-600 mb-1">{new Date(item.created_at).toLocaleString('default', { month: 'short' })}</p><h4 className="text-2xl lg:text-3xl font-black text-white leading-none tracking-tighter">{new Date(item.created_at).getDate()}</h4></div>
                ...
                <AnimatePresence>{expandedId === item.id && (<motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="border-t border-white/5 bg-black/20"><div className="p-6 lg:p-10 grid grid-cols-1 md:grid-cols-2 gap-8 lg:gap-10"><div className="space-y-6"><DetailBox label="Technical Description" value={item.summary || item.description} />{(item.details || item.notes) && <DetailBox label="Narrative Logs" value={item.details || item.notes} />}</div><div className="space-y-6"><div className="bg-white/5 p-6 rounded-3xl border border-white/5"><p className="text-[9px] font-black uppercase text-zinc-600 mb-4 tracking-widest italic text-center">Repository Artifacts</p><div className="flex flex-wrap justify-center gap-4">{item.drive_link && <ArtifactLink label="Drive Assets" icon={<Globe size={14}/>} url={item.drive_link} />}{item.github_link && <ArtifactLink label="Source Repo" icon={<Code2 size={14}/>} url={item.github_link} />}{item.doc_link && <ArtifactLink label="Documentation" icon={<FileText size={14}/>} url={item.doc_link} />}</div></div><div className="bg-white/[0.01] p-6 rounded-3xl border border-dashed border-white/5 text-center"><p className="text-[9px] font-black uppercase text-zinc-700 mb-1 tracking-widest">Scholar Signature</p><p className="text-xs font-bold text-zinc-500 italic">{item.student_name} (ID: {item.student_id?.slice(0,8)})</p></div></div></div></motion.div>)}</AnimatePresence>
              </motion.div>
            ))
          )}
        </div>
