...
import { 
  ...
  ExternalLink, Link, Code2, Lock, Menu 
} from 'lucide-react';
...
export default function AdminDashboard() {
  ...
  async function fetchSystemVolume() {
    ...
    try {
      ...
      const combined = [...(pb?.map(p => ({ ...p, sys_cat: 'publication' })) || []), ...(pr?.map(p => ({ ...p, sys_cat: 'project' })) || [])].sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime());
      setUsers(u || []);
      setSubmissions(combined);
      setStats({ scholars: u?.filter(x => x.role === 'student').length || 0, mentors: u?.filter(x => x.role === 'faculty').length || 0, journals: pb?.filter(x => x.pub_type === 'journal').length || 0, patents: pb?.filter(x => x.pub_type === 'patent').length || 0, confs: pb?.filter(x => x.pub_type === 'conference').length || 0, ongoing: pr?.filter(x => x.category === 'ongoing').length || 0, course: pr?.filter(x => x.category === 'course').length || 0, papers: pr?.filter(x => x.category === 'papers').length || 0 });
    ...
    <div className="flex flex-col lg:flex-row min-h-screen bg-[#09090B] text-[#FAFAFA] font-sans selection:bg-emerald-500/30 overflow-x-hidden">
      ...
      <AnimatePresence>
        {(showMobileSidebar || (typeof window !== 'undefined' && window.innerWidth >= 1024)) && (
          <motion.nav 
            ...
            className={`fixed lg:relative z-[70] w-72 h-full border-r border-white/5 bg-[#0C0C0E] p-8 flex flex-col flex-shrink-0 transition-transform lg:translate-x-0 ${showMobileSidebar ? 'translate-x-0' : '-translate-x-full lg:flex'}`}
          >
            <button onClick={() => setShowMobileSidebar(false)} className="lg:hidden absolute top-8 right-8 text-zinc-500"><X size={28}/></button>
            <div className="flex items-center gap-4 mb-14 px-2 hidden lg:flex"><div className="w-12 h-12 bg-emerald-600 rounded-2xl flex items-center justify-center shadow-xl"><ShieldCheck size={26} className="text-white" /></div><span className="text-2xl font-black tracking-tighter uppercase leading-none italic">DBMS<br/><span className="text-emerald-500">ONE</span></span></div>
      ...
      <main className="flex-1 overflow-y-auto p-6 lg:p-16 relative">
        <header className="flex flex-col xl:flex-row justify-between items-start xl:items-center mb-12 gap-10">
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}><h1 className="text-6xl lg:text-8xl font-black tracking-tighter italic text-white leading-none">ROOT.</h1><p className="text-zinc-500 text-lg lg:text-xl font-medium mt-3 uppercase tracking-[0.3em] italic">System Governance Registry</p></motion.div>
          <button onClick={() => setShowModal(true)} className="w-full lg:w-auto bg-emerald-600 hover:bg-emerald-500 text-white px-10 py-5 rounded-[32px] font-black text-xl shadow-2xl transition-all active:scale-95 flex items-center justify-center gap-3"><UserPlus size={24} strokeWidth={3} /> Provision Access</button>
        </header>
            ...
            {activeTab === 'uploads' ? (
              <div className="space-y-4">
                {submissions.filter(i => (i.title + i.student_name + i.mentor_name).toLowerCase().includes(searchTerm.toLowerCase())).map((item, i) => (
                    <div key={i} className="bg-white/[0.01] border border-white/5 rounded-[24px] lg:rounded-[32px] overflow-hidden group">
                      <div onClick={() => setExpandedId(expandedId === item.id ? null : item.id)} className="p-6 lg:p-8 flex justify-between items-center cursor-pointer hover:bg-white/[0.03] transition-all">
