...
export default function AdminDashboard() {
  ...
  async function fetchSystemVolume() {
    ...
    try {
      ...
      setStats({
        ...
        journals: pubs?.filter(x => x.pub_type === 'journal').length || 0,
        patents: pubs?.filter(x => x.pub_type === 'patent').length || 0,
        confs: pubs?.filter(x => x.pub_type === 'conference').length || 0,
        ongoing: projs?.filter(x => x.category === 'ongoing').length || 0,
        course: projs?.filter(x => x.category === 'course').length || 0,
        papers: projs?.filter(x => x.category === 'papers').length || 0,
      });
  ...
  const handleProvision = async (e: React.FormEvent) => {
    ...
    const { error } = await supabase.from('users').insert([{ 
      ...formData, role: creationMode, temp_pass: 'WELCOME@123' 
    }]);
    if (!error) {
      setShowModal(false);
      ...
      alert("Provisioning Error: Identify duplicate or system fault.");
    }
    setSubmitting(false);
  };

  if (loading) return <div className="min-h-screen bg-[#09090B] flex items-center justify-center text-emerald-500 font-black text-2xl animate-pulse italic uppercase tracking-tighter">Syncing with ONE...</div>;
      ...
      <main className="flex-1 overflow-y-auto p-10 lg:p-16 relative">
        <header className="flex flex-col xl:flex-row justify-between items-start xl:items-center mb-12 gap-10">
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
            <h1 className="text-8xl font-black tracking-tighter italic text-white leading-none">ONE Hub.</h1>
            <p className="text-zinc-500 text-xl font-medium mt-3 uppercase tracking-[0.3em] italic">System Governance Registry</p>
          </motion.div>
          <button onClick={() => setShowModal(true)} className="bg-emerald-600 hover:bg-emerald-500 text-white px-10 py-5 rounded-[32px] font-black text-xl shadow-2xl transition-all active:scale-95 flex items-center gap-3">
              <UserPlus size={24} strokeWidth={3} /> Provision Access
          </button>
        </header>

        {/* 📊 STATS GRID */}
        <div className="grid grid-cols-2 lg:grid-cols-4 xl:grid-cols-8 gap-4 mb-12">
          <StatBox label="Scholars" value={stats.scholars} color="emerald" />
          <StatBox label="Mentors" value={stats.mentors} color="blue" />
          <StatBox label="Journals" value={stats.journals} color="emerald" />
          <StatBox label="Patents" value={stats.patents} color="orange" />
          <StatBox label="Confs" value={stats.confs} color="blue" />
          <StatBox label="Ongoing" value={stats.ongoing} color="orange" />
          <StatBox label="Course" value={stats.course} color="blue" />
        ...
        <div className="bg-[#121214] border border-white/5 rounded-[48px] overflow-hidden shadow-2xl">
          <div className="p-10 flex flex-col md:flex-row justify-between items-center gap-8 border-b border-white/5 bg-white/[0.01]">
            <h3 className="text-3xl font-black text-white italic uppercase tracking-tighter">
              {activeTab === 'uploads' ? 'Universal Feed' : 'Registry Control'}
            </h3>
            <div className="relative w-full md:w-[450px]">
              <Search className="absolute left-6 top-1/2 -translate-y-1/2 text-zinc-600" size={24} />
              <input 
                className="w-full bg-white/5 border border-white/10 rounded-[28px] py-5 pl-16 pr-8 text-xl text-white outline-none focus:ring-2 focus:ring-emerald-500/20 placeholder:text-zinc-800" 
                placeholder={`Search ONE Registry...`} 
                    ...
                    <div key={i} className="bg-white/[0.01] border border-white/5 rounded-[32px] overflow-hidden group">
                      ...
                      >
                        <div className="flex items-center gap-8">
                          <div className={`w-16 h-16 rounded-2xl flex items-center justify-center font-black text-xs uppercase ${item.sys_cat === 'publication' ? 'bg-emerald-500/10 text-emerald-500' : 'bg-blue-500/10 text-blue-500'}`}>
                              {item.pub_type?.[0] || item.category?.[0]}
                          </div>
                          <div>
                              <div className="flex gap-3 mb-2">
                                <span className="text-[10px] font-black uppercase tracking-widest text-emerald-500 bg-emerald-500/5 px-2 py-0.5 rounded">{item.pub_type || item.category}</span>
                                <span className="text-[10px] font-black uppercase tracking-widest text-zinc-600">{new Date(item.created_at).toLocaleDateString()}</span>
                              </div>
                              <h4 className="text-3xl font-black text-white uppercase italic leading-tight">{item.title}</h4>
                              <p className="text-sm text-zinc-500 mt-2 font-bold uppercase italic">Scholar: {item.student_name} • Mentor: {item.mentor_name}</p>
                          </div>
                        </div>
                        <div className="flex items-center gap-4">
                          <button onClick={(e) => { e.stopPropagation(); handleDelete(item.sys_cat === 'publication' ? 'publications' : 'projects', item.id); }} className="p-4 bg-zinc-900 rounded-2xl text-zinc-700 hover:text-red-500 transition-all opacity-0 group-hover:opacity-100">
                              <Trash2 size={24} />
                          </button>
                          ...
                          <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="bg-black/20 border-t border-white/5">
                            <div className="p-10 grid grid-cols-1 md:grid-cols-2 gap-10">
                              <div className="space-y-6">
                                ...
                                <DetailItem label="Mentor Credentials" value={`${item.mentor_name} (${item.mentor_email})`} />
                                {item.associated_project && <DetailItem label="Linked Project" value={item.associated_project} />}
                              </div>
                              <div className="space-y-6">
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                   <MiniDataBox label="Date Logged" val={new Date(item.created_at).toLocaleDateString()} />
                                   <MiniDataBox label="Track Type" val={item.pub_type || item.category} />
                                </div>
                              </div>
                            </div>
                          </motion.div>
                  ...
                  <div key={i} className="bg-white/[0.02] border border-white/5 p-8 rounded-[40px] group transition-all hover:border-emerald-500/20">
                     <div className="flex justify-between items-start mb-6">
                        <div className="flex items-center gap-6">
                           <div className="w-16 h-16 bg-zinc-800 rounded-3xl flex items-center justify-center text-emerald-500 font-black text-3xl shadow-inner border border-white/5">
                              {u.name[0]}
                           </div>
                           <div>
                              <h3 className="text-2xl font-black text-white italic uppercase leading-none tracking-tighter">{u.name}</h3>
                              <p className="text-zinc-500 text-[10px] font-black uppercase tracking-[0.3em] mt-2">{u.role} • {u.dept}</p>
                           </div>
                        </div>
                     ...
                     <details className="group/details">
                        <summary className="list-none cursor-pointer flex items-center justify-between w-full py-4 px-6 bg-white/5 rounded-2xl border border-white/5 text-[10px] font-black uppercase text-emerald-500 group-open:mb-4 transition-all">
                           <span>View Upload Profile</span>
                        ...
                        <div className="space-y-2 mt-4 max-h-48 overflow-y-auto pr-2 custom-scrollbar">
                           {submissions.filter(s => u.role === 'student' ? s.student_id === u.user_id : s.mentor_email === u.email).map((entry, idx) => (
                              <div key={idx} className="p-4 bg-white/5 rounded-xl border border-white/5 flex justify-between items-center group/item">
                                 <div>
                                    <p className="text-xs font-bold text-zinc-300 uppercase italic">{entry.title}</p>
                                    <p className="text-[8px] font-black text-zinc-600 uppercase mt-1">{entry.pub_type || entry.category}</p>
                                 </div>
            ...
            <div className="fixed inset-0 z-50 flex items-center justify-center p-6 backdrop-blur-3xl bg-black/80">
              <motion.div initial={{ scale: 0.95, y: 20 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.95, y: 20 }} className="bg-[#121214] border border-white/10 w-full max-w-2xl rounded-[60px] p-12 relative shadow-2xl">
                 ...
                 <form onSubmit={handleProvision} className="space-y-6">
                    <Input icon={<Users size={20}/>} label="Full Identity Name" onChange={(e:any) => setFormData({...formData, name: e.target.value})} />
                    <Input icon={<Mail size={20}/>} label="Academic Email" onChange={(e:any) => setFormData({...formData, email: e.target.value})} />
                    <Input icon={<Database size={20}/>} label="Department" onChange={(e:any) => setFormData({...formData, dept: e.target.value})} />
                    <button type="submit" disabled={submitting} className={`w-full py-8 rounded-[40px] font-black text-2xl text-white mt-6 shadow-2xl transition-all ${creationMode === 'student' ? 'bg-emerald-600' : 'bg-blue-600'}`}>Sync & Finalize</button>
                 </form>
              </motion.div>
            </div>
          )}
...
function DetailItem({ label, value }: { label: string, value: string }) {
  return (
    <div className="space-y-2">
      <p className="text-[9px] font-black uppercase text-zinc-700 tracking-[0.2em] italic">{label}</p>
      <div className="bg-white/5 border border-white/10 p-5 rounded-2xl text-sm italic text-zinc-400 leading-relaxed shadow-inner">
        {value || "No detailed log found."}
      </div>
    </div>
  );
}
...
function MiniDataBox({ label, val }: any) {
  return (
    <div className="bg-white/[0.02] border border-white/5 p-4 rounded-2xl">
       <p className="text-[8px] font-black uppercase text-zinc-700 mb-1">{label}</p>
       <p className="text-xs font-bold text-zinc-400 italic uppercase">{val}</p>
    </div>
  );
}
...
function StatBox({ label, value, color }: any) {
  const themes: any = { emerald: "text-emerald-500", blue: "text-blue-500", orange: "text-orange-500", rose: "text-rose-500" };
  return (
    <div className="bg-[#121214] border border-white/5 p-6 rounded-[32px] shadow-xl relative overflow-hidden group">
      <p className="text-zinc-600 text-[8px] font-black uppercase tracking-widest relative z-10 italic mb-2 leading-none">{label}</p>
      <h4 className={`text-4xl font-black tracking-tighter relative z-10 ${themes[color]}`}>{value}</h4>
    </div>
  );
}
